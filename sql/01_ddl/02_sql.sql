DROP SCHEMA IF EXISTS cryptotrade CASCADE;
...
CREATE INDEX idx_portefeuille_user ON portefeuilles(utilisateur_id);
...
CREATE INDEX idx_ordres_user_date ON ordres (utilisateur_id, date_creation DESC);
CREATE INDEX idx_ordres_paire_date ON ordres (paire_id, date_creation DESC);
...
-- Ajout des indexes
CREATE INDEX idx_trades_paire_date ON trades (paire_id, date_execution DESC);
...
CREATE INDEX idx_prix_last ON prix_marche (paire_id, date_maj DESC);
